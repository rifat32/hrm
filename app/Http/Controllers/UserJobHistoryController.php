          ...
          try {
              ...
              ->when(!empty($request->search_key), function ($query) use ($request) {
                  ...
                  ->when(!empty($request->start_date), function ($query) use ($request) {
                      return $query->where('user_job_histories.created_at', ">=", Carbon::createFromFormat('d-m-Y', ($request->start_date)));
                  })
                  ->when(!empty($request->end_date), function ($query) use ($request) {
                      return $query->where('user_job_histories.created_at', "<=", Carbon::createFromFormat('d-m-Y', ($request->end_date . ' 23:59:59'))->format('Y-m-d'));
                  })
