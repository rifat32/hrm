             ...
             ->when(empty($request->business_tier_id), function ($query) use ($request) {
                return $query->where('modules.business_tier_id', NULL);
                 ...
                 ->when(!empty($request->start_date), function ($query) use ($request) {
                     return $query->where('modules.created_at', ">=", Carbon::createFromFormat('d-m-Y', ($request->start_date)));
                 })
                 ->when(!empty($request->end_date), function ($query) use ($request) {
                     return $query->where('modules.created_at', "<=", Carbon::createFromFormat('d-m-Y', ($request->end_date . ' 23:59:59'))->format('Y-m-d'));
                 })
